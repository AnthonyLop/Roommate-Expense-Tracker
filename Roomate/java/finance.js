function calculateBudget() {
  ...
  // Set the color based on the budget value
  var budgetColor = budget >= 0 ? 'green' : 'red';

  // Display the result with the dynamic color
  document.getElementById('result').innerHTML =
      'Your total income is: $' + totalIncome.toFixed(2) + '<br>' +
      'Your total expenses are: $' + totalExpenses.toFixed(2) + '<br>' +
      'Your budget is: <strong style="color: ' + budgetColor + '">$' + budget.toFixed(2) + '</strong>';
}
...
function downloadResults() {
  ...
  function resetForm() {
    ...
    document.getElementById('expenses1').value = '';
    document.getElementById('expenses2').value = '';
